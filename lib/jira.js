...
var url = require('url'),
  ...
  OAuth = require("oauth"),
  JiraApi;
JiraApi = exports.JiraApi = function (protocol, host, port, username, password, apiVersion, verbose, strictSSL, oauth) {
  'use strict';
...
(function () {
  ...
  // *  sprintIds: array of active sprints to filter by <optional>
  // *  callback: for when it's done
  //
  // ### Returns ###
  //
  ...
  // *  results: the object with the issues and additional sprint information
  /**
   * Returns sprint health
   *
   * @param rapidViewId
   * @param sprintId
   * @param callback
   */
  this.getSprintHealth = function getSprintHealth(rapidViewId, sprintId, callback) {
    var options = {
      rejectUnauthorized: this.strictSSL,
      uri: this.makeUri('/gadgets/sprints/health?rapidViewId=' + rapidViewId + '&sprintId=' + sprintId, 'rest/greenhopper/', '1.0'),
      method: 'GET',
      json: true
    };
    this.doRequest(options, function (error, response) {
  ...
  this.addIssueToSprint = function (issueId, sprintId, callback) {
    var options = {
      rejectUnauthorized: this.strictSSL,
      uri: this.makeUri('/sprint/' + sprintId + '/issues/add', 'rest/greenhopper/', '1.0'),
      method: 'PUT',
  ...
  this.getBacklogForRapidView = function (rapidViewId, callback) {
    var options = {
      rejectUnauthorized: this.strictSSL,
      uri: this.makeUri('/xboard/plan/backlog/data?rapidViewId=' + rapidViewId, 'rest/greenhopper/', '1.0'),
      method: 'GET',
      json: true
    };
    this.doRequest(options, function (error, response) {
